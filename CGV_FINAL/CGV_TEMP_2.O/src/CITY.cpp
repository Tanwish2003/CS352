...
GLuint texture1, texture2; // Declare two textures
...
{
  // Draw the stand
  glColor3f(0.741, 0.718, 0.420); // Set the color to grey
  glBegin(GL_QUADS);
  glVertex3f(x + width / 2 - 10, 0, z);
  glVertex3f(x + width / 2 + 10, 0, z);
  glVertex3f(x + width / 2 + 10, y, z);
  glVertex3f(x + width / 2 - 10, y, z);
  glEnd();
  ...
  // Calculate the index of the texture to use
  // Change 4000 to the number of milliseconds you want each image to remain
  int textureIndex = (currentTime / 4000) % 2;

  // Use the selected texture
  if (textureIndex == 0)
  ...
  glVertex3f(x + width, y, z); // Swap the x-coordinate
  glTexCoord2f(0.0f, 0.0f);
  glVertex3f(x + width, y + height, z); // Swap the x-coordinate
  glTexCoord2f(1.0f, 0.0f);
  glVertex3f(x, y + height, z); // Swap the x-coordinate
  glEnd();
...
void makeBuildings()
{
  ...
  loadTextures();
  drawBillboard(-500, 100, -500, 200, 100);
  // drawText(-500, 100, -500, "Welcome to our city!");
